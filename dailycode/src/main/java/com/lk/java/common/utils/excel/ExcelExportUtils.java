...
public class ExcelExportUtils {
    ...
    private  static final int THREADHOLD = 1000;
    public static <T> boolean excelExportDivide(Map<String, String> maps, List<T> list, File file, String sheetName) {
    	//拆分
		...
		if (type.equals(FILE_EXTENSION_XLSX)) {
			wb = new XSSFWorkbook();
//			XSSFWorkbook xsswb = new XSSFWorkbook();
//			wb = new SXSSFWorkbook(xsswb, 500);
		}
    	
    	if(length < THREADHOLD) {
    		new Thread(new ExportExcel(maps, list, file, sheetName + 1, wb)).start();
    		return true;
    	}else {
    		int num = length % THREADHOLD == 0 ? (int)(length/THREADHOLD) : (int)(length/THREADHOLD) + 1;
    		for (int i = 0; i < num; i++) {
    			int end = (int) ((i + 1) * THREADHOLD < length ? (i + 1) * THREADHOLD : length);
    			List<T> subList = list.subList(i * THREADHOLD, end);
    			exportExcel(maps, subList, file, sheetName + (i + 1), wb);
			}
    		return true;
    	}
    }
...
//    }
	...
	private static <T> void exportExcel(Map<String, String> maps, List<T> list, File file, String sheetName, Workbook wb){
